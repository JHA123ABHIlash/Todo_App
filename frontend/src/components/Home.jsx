  ...
  return (
    <div className='my-10 bg-gray-100 max-w-lg lg:max-w-xl rounded-lg shadow-lg mx-8 sm:mx-auto p-6'>
        ...
        {loading ? (<div className='text-center justify-center'>
            <span className='text-grey-500'>Loading...</span></div>)
        :error?(<div className='text-center text-red-600 font-semibold'>{error}</div>)
        :(
            <ul className='space-y-2'>
                    {todos.map((todo,index)=>(
                        <li key={todo._id || index} className='flex items-center justify-between p-3 bg-grey-100 rounded-md'>
                            <div className='flex items-center'>
                                <input type="checkbox" className='mr-2' checked={todo.completed} onChange={()=>todoStatus(todo._id)}/>
                                <span className={`${todo.completed ? "line-through text-grey-800 font-semibold" : ""}`}>{todo.text}</span>
                            </div>
        ...
        <p className='mt-4 text-center text-sm text-grey-700'>{remainingTodos} remaining todo</p>
